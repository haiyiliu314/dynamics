  ...
  double precision, parameter                  ::t_end = 15.0d0, &
                                               dt = t_end/dble(Nt)
  ...
  function funcp(nt_via, f_via, p_via)
    ...
    funcp = -(0.0d0,1.0d0)*(y*y*p_via - ((2.0d0*f_sum)*p_via) + shift*p_via - (0.0d0,1.0d0) * gamma * p_via/Ebind - (1.0d0-2.0d0*f_via)*(p_sum+Etime(nt_via)))/hbar*Ebind
  end function funcp
  ...
  subroutine RK_pf(f, g, f1, g1, n)
    ...
    kf1 = dt * funcf(n,g1)
    kg2 = dt * funcp(n+0.50d0, f1 + kf1/2.0d0, g1 + kg1/2.0d0)
