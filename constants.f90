  ...
  integer, parameter                           ::Nt = 500000d0
  double precision, parameter                  ::f0 = 0d0, t_end = 25d0, &
                                               dt = t_end/dble(Nt)
  integer, parameter                           ::Ny = 400d0, N_fine = 50d0, Nphi = 100d0
  double precision, parameter                  ::ymax = 40d0, dy = ymax/Ny
  double precision, parameter                  ::pi = 4d0*atan(1.), hbar = 4.135667662d0/2d0/pi, &
                                               e = 1.60217662d-19
  double precision, parameter                  ::Ebind = 4.18d0*e, gamma = 0.39d0*e
  double precision, parameter                  ::sigmat = 0.15d0, tstart = -10d0, & 
                                               E_excit = 1d-3, shift = 4d0!, tstart = -3d0*sigmat
  double precision                             ::coul_mat(Ny, Ny), Et, time(Nt+1), y(Ny)=0, &
                                               y_fine(N_fine), dy_fine, f(Ny) = 0, ft(Nt+1)
  complex*16                                   ::p(Ny) = 0, pt(Nt+1)
  ...
  double precision function Etime(tvia)
    ...
    Etime = E_excit*dexp(-(tvia*dt+tstart)*(tvia*dt+tstart)/(sigmat*sigmat))
  end function Etime
  ...
  subroutine coul_matrix
    ...
    do Ndo = 1d0, Ny
      ...
      do Ndo_in = 1d0, Nphi
        coul_mat(:, Ndo)=coul_mat(:,Ndo)+(1d0/(sqrt(y(Ndo)*y(Ndo)+y*y-2d0*y*y(Ndo)*cos (b(Ndo_in)))))*2d0*y*w1(Ndo_in)
      end do
      coul_mat(Ndo, Ndo) = 0d0
      do Ndo_in = 1d0, N_fine
        coul_mat(Ndo,Ndo)=coul_mat(Ndo,Ndo) + sum( 1d0 / sqrt( y(Ndo) *y(Ndo) + y_fine(Ndo_in) *y_fine(Ndo_in) - 2d0 * y_fine(Ndo_in) * y(Ndo) * cos( b ) ) / N_fine * 2d0 * y_fine(Ndo_in)  * w1)
      end do
    end do
