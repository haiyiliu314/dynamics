  ...
  function funcp(nt_via, f_via, p_via)
    ...
    complex*16, dimension(Ny, 1)                           ::p_via    
    double precision, dimension(Ny, 1)                     ::f_via
    complex*16, dimension(Ny, 1)                           ::funcp
    funcp = -(0d0,1d0)*(y**2*p_via - (((2d0*matmul(coul_mat,fvia))*p_via) + shift*p_via - (0d0,1d0) * gamma * p_via/Ebind - (1d0-2d0*f_via)*(matmul(coul_mat, p_via)+Etime(nt_via))))/hbar*Ebind
  end function funcp
  function funcf(nt_via, p_via)
    double precision                                    ::nt_via 
    complex*16, dimension(Ny)                           ::p_via    
    double precision, dimension(Ny)                     ::funcf
    funcf = aimag(conjg(Et(nt_via)+matmul(coul_mat,p_via))*p_via*2d0)/hbar*Ebind
  end function funcf
