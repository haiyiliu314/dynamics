  ...
  do i1 = 1,N_freq
 !   freqgrid(i1) =0.001d0*i1-0.0005d0+0.4
    freqgrid(i1) =0.001d0*i1-0.0005d0
!    freqgrid(i1) =1d0+0.01d0*i1-0.005d0
  end do
!-----------output the frequency grid---------------
  freqgrid = (freqgrid*Eg)/hbar
...
!------------end output-----------------------------
  ...
  !function for calculating electrical field(unit: binding energy)
    ...
    Etime = -A_excit*Ebind*exp(-t1*t1/(sigmat_A*sigmat_A)) * (A_freq_para*sin(A_freq_para*t1) + 2d0*t1/(sigmat_A*sigmat_A) * cos(A_freq_para*t1))/exp((0d0, 1d0)*omega_1s*t1)!no E_bind non RWA
!    Etime = A_excit*Ebind*exp(-t1*t1/(sigmat_A*sigmat_A)) * (A_freq_para*sin(A_freq_para*t1))/exp((0d0, 1d0)*omega_1s*t1)!no E_bind non RWA just sinusoidal part
!    Etime = E_excit*exp(-(t1)*(t1)/(sigmat*sigmat)) * (1d0 + 0d0*exp(-(0d0,1d0)*2d0*omega_1s*t1))!no E_bind   RWA
  end function Etime
...
!    Atime = A_excit*exp(-((tvia)*dt+tstart_A)*((tvia)*dt+tstart_A)/(sigmat_A*sigmat_A))*(exp((0d0, 1d0)*A_freq_para*t1/hbar)+exp(-(0d0, 1d0)*A_freq_para*t1/hbar))*Ebind        ! unit:meV    
    Atime = A_excit*exp(-t1*t1/(sigmat_A*sigmat_A))*(cos(A_freq_para*t1))*Ebind        ! unit:meV                    
  end function Atime
...
!        write(700, format_V) aimag(conjg(p_via(-Ndo_m+Ndo_m1+(Nm_o+1), :))*p_sum_part)
      ...
      f_out(Ndo_m, :) = (conjg(Etime(nt_via))*p_via(Ndo_m, :) - Etime(nt_via)*conjg(p_via((2*Nm_o+2)-Ndo_m, :)) &
                        +(pp_sum_plus-pp_sum)- (0.0d0,1.0d0) *(dble(Ndo_m==Nm_o+1)+1)*gamma &
                        * f_via(Ndo_m, :)/Ebind)&
                        /hbar*Ebind*dt/(0.0d0, 1.0d0)
    end do
    J_THZ_t1 = 0d0
    J_THZ_t1 = dy*sum(y*y*(f_via(Nm_o, :)+f_via(Nm_o+2, :)))/(2.0d0*pi)
    kE_out = dt * Etime(nt_via) * Ebind * exp((0.0d0, 1.0d0)*(dt*nt_via+tstart_A)*freqgrid)  
